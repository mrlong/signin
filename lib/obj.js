...
//
// 获取临时二维码的环境sceneId号,生成9位不重复的数字
// type 表示类型
//
obj.prototype.newTmpSceneId = function(type){
  var d = new Date();
  var code = type + '' ;
  code += (d.getHours()<10  ?Math.ceil(Math.random()*10)  + d.getHours():d.getHours()+'') +
          (d.getMinutes()<10?Math.ceil(Math.random()*10)  + d.getMinutes():d.getMinutes()+'') + 
          (d.getSeconds()<10?Math.ceil(Math.random()*10)  + d.getSeconds():d.getSeconds()+'');
  code +=   Math.ceil(Math.random()*10) + '' + Math.ceil(Math.random()*10) ;
